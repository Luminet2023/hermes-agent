...
COMMAND_REGISTRY: list[CommandDef] = [
    ...
    CommandDef("save", "保存当前对话", "会话",
               cli_only=True),
    CommandDef("retry", "重试最后一条消息（重新发送给代理）", "会话"),
    CommandDef("undo", "移除最后一次用户/助手的对话", "会话"),
    ...
    CommandDef("rollback", "列出或恢复文件系统检查点", "会话",
               args_hint="[number]"),
    CommandDef("snapshot", "创建或恢复 Hermes 配置/状态的快照", "会话",
    ...
    CommandDef("btw", "使用会话上下文的短期支线问题（无工具，不持久化）", "会话",
               args_hint="<question>"),
    CommandDef("agents", "显示活动的代理和正在运行的任务", "会话",
               aliases=("tasks",)),
    CommandDef("queue", "为下一轮排队提示词（不打断当前内容）", "会话",
               aliases=("q",), args_hint="<prompt>"),
    CommandDef("steer", "在这轮工具调用后注入消息（不打断执行）", "会话",
               args_hint="<prompt>"),
    ...
    CommandDef("config", "显示当前配置", "配置",
               cli_only=True),
    CommandDef("model", "切换当前会话的模型", "配置", args_hint="[model] [--provider name] [--global]"),
    CommandDef("provider", "显示可用提供商及当前提供商",
               "配置"),
    CommandDef("gquota", "显示 Google Gemini Code Assist 配额使用情况", "信息"),

    CommandDef("personality", "设置预定义的角色性格", "配置",
               args_hint="[name]"),
    CommandDef("statusbar", "切换上下文/模型状态栏的显示", "配置",
               cli_only=True, aliases=("sb",)),
    CommandDef("verbose", "循环切换工具进度显示：关闭 -> 新建 -> 全开 -> 详细",
               "配置", cli_only=True,
               gateway_config_gate="display.tool_progress_command"),
    CommandDef("yolo", "切换 YOLO 模式（跳过所有危险命令的批准）",
               "配置"),
    CommandDef("reasoning", "管理推理强度和显示", "配置",
               args_hint="[level|show|hide]",
               subcommands=("none", "minimal", "low", "medium", "high", "xhigh", "show", "hide", "on", "off")),
    CommandDef("fast", "切换快速模式 — OpenAI Priority Processing / Anthropic Fast Mode (Normal/Fast)", "配置",
               args_hint="[normal|fast|status]",
               subcommands=("normal", "fast", "status", "on", "off")),
    CommandDef("skin", "显示或更改显示的皮肤/主题", "配置",
               args_hint="[name]"),
    ...
    CommandDef("restart", "在正在运行的任务结束后优雅地重启网关", "会话",
               gateway_only=True),
    ...
    CommandDef("copy", "复制最后一次助手回复到剪贴板", "信息",
               cli_only=True, args_hint="[number]"),
    CommandDef("paste", "附加剪贴板中的图片", "信息",
               cli_only=True),
    CommandDef("image", "为下一个提示词附加本地图片文件", "信息",
